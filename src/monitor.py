...
TARGET_COL = 'demand'
DRIFT_THRESHOLD = 0.09  # trigger if >9% of columns drift (i.e. even 1 out of 10)
...
def simulate_production_data(reference_df, drift=False):
    ...
    if drift:
        ...
        print("✓ Simulating NORMAL production data")
    return df
...
def run_monitor(drift=False):
    ...
    if col_scores:
        print("\nPer-column drift scores (threshold=0.1):")
    ...
    if drift_detected:
        ...
        print("\n✅ No significant drift. Model is healthy.")
    print(f"{'='*40}")
